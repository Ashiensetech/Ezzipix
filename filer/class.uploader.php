    ...
    private function prepareFiles() {
        ...
        if ($validate) {
            ...
            for ($i = 0; $i < count($field['name']); $i++) {
                ...
                $metas['name']      = $this->generateFileName($this->options['title'], ['name' => $metas['old_name'], 'size' => $metas['size']]) . (!empty($metas['extension']) ? "." . $metas['extension'] : "");
                $metas['file']      = $this->options['uploadDir'] . $metas['name'];
                $metas['replaced']  = file_exists($metas['file']);
                $metas['date']      = date('r');

                if (!in_array($field['name'][$i], $removed_files) && $this->validate(array_merge($metas, ['index' => $i, 'tmp' => $tmp_name])) && $this->uploadFile($tmp_name, $metas['file'])) {
